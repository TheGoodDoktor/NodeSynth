...
struct FSidEmulator::FImpl {
  ...
  void TickOneCycle(std::vector<FSidRegisterWrite> *OutWrites,
                    uint16_t SampleOffset, float *AccumSample,
                    uint32_t *AccumCount) {
    ...
    if (Pins & M6581_SAMPLE) {
      if (AccumSample != nullptr) {
        *AccumSample += Sid.sample;
      }
      if (AccumCount != nullptr) {
        *AccumCount += 1;
      }
    }
...
FSidEmulator::FSidEmulator() : Impl(std::make_unique<FImpl>()) {}
...
void FSidEmulator::Reset(double ChipClockHz, double SampleRateHz) {
  ...
  Impl->CyclesPerSampleQ16 =
      static_cast<uint32_t>((ChipClockHz / SampleRateHz) * 65536.0);
...
void FSidEmulator::LoadIntoRam(const uint8_t *Data, size_t Size,
                               uint16_t LoadAddr) {
...
void FSidEmulator::PokeRam(uint16_t Addr, uint8_t Value) {
  Impl->Ram[Addr] = Value;
}
...
uint8_t FSidEmulator::GetA() const {
  return m6502_a(const_cast<m6502_t *>(&Impl->Cpu));
}
uint16_t FSidEmulator::GetPC() const {
  return m6502_pc(const_cast<m6502_t *>(&Impl->Cpu));
}

float FSidEmulator::TickOneAudioSample(
    std::vector<FSidRegisterWrite> &OutWrites, uint16_t SampleOffset) {
  ...
  return (AccumCount > 0) ? (AccumSample / static_cast<float>(AccumCount))
                          : 0.0f;
}
...
bool FSidEmulator::RunInitRoutine(uint16_t InitAddr, uint8_t Subtune,
...
void FSidEmulator::SetVbiTimer(bool bEnabled, double TickRateHz) {
  ...
  if (bEnabled && TickRateHz > 0.0) {
    Impl->VbiCyclesPerTick =
        static_cast<uint32_t>(Impl->ChipClockHz / TickRateHz);
  } else {
