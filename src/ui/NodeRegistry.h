#pragma once

#include <memory>
#include <vector>

#include "dsp/Node.h"

namespace NodeSynth
{
	// One entry per concrete node type. TypeName matches INode::GetTypeName() so
	// it can drive the icon lookup; MenuLabel is the human-readable string shown
	// in the create-node menu and the node palette; Description is the tooltip
	// shown on hover in the palette; Category drives the collapsible grouping in
	// the palette UI (palette draws nodes grouped by Category in registry order);
	// Make builds a fresh instance.
	struct FNodeRegistration
	{
		const char* TypeName;
		const char* MenuLabel;
		const char* Description;
		const char* Category;
		std::shared_ptr<INode> (*Make)();
	};

	const std::vector<FNodeRegistration>& GetNodeRegistry();

	// Construct a node by its type-name string (matches INode::GetTypeName()).
	// Returns nullptr if the type name is not registered. Used by the patch
	// serializer.
	std::shared_ptr<INode> MakeNodeByTypeName(const std::string& TypeName);
}
