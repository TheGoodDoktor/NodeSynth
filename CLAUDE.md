...
- **Spectral:** Vocoder (channel vocoder — 8/16/24-band analysis/synthesis filter banks, two Audio inputs: Carrier + Modulator, formant shift).
- **Routing / math:** Gain, VCA, Mixer, Add, Multiply, Scale.
- **Control / sequencing:** Clock (BPM gate), Sequencer (16-step external-clock-driven), Arpeggiator (monophonic note sink — Up/Down/UpDown/DownUp/AsPlayed/Random, 1–4 octaves, internal BPM/Rate clock or external Clock-input sync, gate length, swing, latch), Gate (manual toggle), MIDI Input, MIDI CC (smoothed Control output + Learn), Virtual Keyboard.
...
Catch2 test harness (~336 `TEST_CASE`s across ~40 test files) covers the graph, smoother, every DSP node, the SPSC rings (MIDI note / CC + mic capture) under concurrent producer/consumer, command-queue dispatch + `SetParam` fan-out to per-voice clones, patch round-trip serialisation, `INode::Clone()` for every cloneable type, the per-voice flag, voice-allocator stealing policy (oldest fully-released → tailing → held), the arpeggiator (held-note capture, all six patterns, octave expansion, internal/external clock, gate window, latch, swing, mid-run mutation + bundled-preset load), the Compile partition with 8-voice polyphonic audio summation and independent per-voice ADSR release, every Phase 4 effect, the full effects roadmap (dynamics, EQ, tremolo/auto-pan, character set), Phaser, Wavetable, Oversampler, Modulation Matrix, MIDI CC mapping, the Vocoder band-pass primitive + analysis→synthesis spectral routing, the Mic Input ring drain/underrun, the SID emulator + PSID loader, and undo/redo. Audio buffers are 2-channel (`float[NumOutputs][NumChannels=2][BlockSize]`) with wire-level broadcast (mono producer's L buffer aliases the consumer's R input pointer); `INode::IsOutputStereo(port)` selects L→both broadcast vs paired L→L,R→R plumbing per link; the audio callback reads both sink channels to device L+R speakers.
...
**Vocoder + Mic Input (shipped on `feature/vocoder`, not yet merged):** `FVocoder` (palette category **Effects**) is a classic channel vocoder with two Audio inputs — **Carrier** (port 0, a harmonically-rich synth) and **Modulator** (port 1, classically a voice). The modulator (summed to mono) is split by a log-spaced analysis band-pass bank (8/16/24 bands, 4-pole = two cascaded `FBiquadCoeffs::SetBandpass` biquads); a per-band `FEnvelopeFollower` tracks each band; the carrier is split by a matching synthesis bank (per-channel, so a stereo carrier keeps its image) and each band scaled by its modulator envelope, then summed. Params: Bands, Attack, Release, Formant (synth-bank retune), Mix, Output makeup gain. Fixed `MaxBands=24` arrays → zero `Process` allocation. `FMicInput` (palette **Sources**) is a live audio-capture source: it owns its own miniaudio capture device (separate from the playback device in `main.cpp`), and its capture-thread callback pushes mono samples into a lock-free `FMicRing` (`src/audio/MicRing.h`) that `Process` drains — the same producer/consumer shape as the MIDI subsystem. All miniaudio state is behind an opaque PIMPL (`FMicInput::FMicDevice` in `MicInput.cpp`) so the header stays miniaudio-free. `SetParamValue` is RT-safe (atomic stores only — the command queue dispatches it on the audio thread); device open/close is a UI-thread-only path (`SetDevice`, driven by the custom `MicInputUI` device picker — the `Device` Choice is `bHidden` so it never rides the generic combo's queue push). Non-cloneable; the device persists across recompiles (`Prepare` leaves device + ring untouched). Loaded presets restore the device *index* but don't auto-open it — the user re-selects in the Mic Input panel (device indices are machine-specific). Bundled `FX/Vocoder Talk` preset. Forbidden combo to watch for: monitoring through speakers feeds back into the mic — use headphones. Deferred to v2: unvoiced/sibilance path, adjustable band Q, envelope freeze, capture/playback sample-rate-tracking resample. See `docs/PLAN-VOCODER.md`. **The backlog is now empty.**
...
- **Command queue (UI → Audio).** A lock-free SPSC ring (`FAudioCommandRing` in `src/graph/AudioCommand.h`, 512-slot) carries `SetParam` / `NoteOn` / `NoteOff` commands from UI → audio. The audio callback drains it via `FAudioGraph::DrainCommands` at the start of each block: `SetParam` dispatches to nodes via an `unordered_map<FNodeId, INode*>` populated by `Compile`; `NoteOn` / `NoteOff` broadcast to every `INoteSink` in `FAudioGraph::NoteSinks` (see Polyphony section). UI-side widgets do a *dual write*: they call `Node->SetParamValue` directly (so sliders track immediately and `GetParamValue` stays consistent) AND push a `SetParam` command (so audio-thread state mutates in queue order alongside other commands — important for save/load replay). Structural commands (`AddNode` / `RemoveNode` / `Connect` / `Disconnect`) are **not** in the queue — those still take the snapshot-swap path. RT-safe structural mutation is a much larger engineering lift and there's no Phase 3 deliverable that needs it.
...
Phase 2 added `src/midi/` for the SPSC `FMidiRing` and `src/dsp/MidiInput.{h,cpp}` for the RtMidi-backed `FMidiInput` node. `src/io/` holds the patch/subgraph serialisers and asset browsers; `src/audio/` holds `MicRing.h` (the mic-capture SPSC ring, added with the Vocoder work). Tests live under `tests/` (`*.cpp` glob-collected into the `nodesynth_tests` Catch2 target — adding a new test file needs a CMake re-configure). The `app/` directory in `docs/PLAN.md` still doesn't exist — don't cite it as if it does.
...
- **`NoteOn` / `NoteOff` commands** broadcast to every **note sink** in the snapshot — `FAudioGraph::NoteSinks` is a `std::vector<INoteSink*>` (`dsp/NoteSink.h`), the interface implemented by `FVoiceAllocator` and `FArpeggiator` (`dsp/Arpeggiator.h` — a monophonic note sink that reorders held notes into a clocked pattern and emits the same Gate/Frequency/Velocity/Note Control outputs, so it drops in where the allocator would go). `Compile` collects every reachable node that `dynamic_cast`s to `INoteSink`. UI-thread sources push through the SPSC ring (drained by `DrainCommands`); the global note-input subsystem (`FMidiDeviceManager`, fed via `SetNoteSinks` on each recompile) calls the sink methods directly from the audio thread (we'd otherwise need MPSC for the ring). Note events are audio-thread-only, so a sink's held-note state needs no locking.
