...
**Next up:** Subgraphs (`docs/PLAN-SUBGRAPHS.md`) — reusable `FSubgraph` nodes wrapping an internal graph with user-declared pins, saved as `.nspg` assets, compiled by inline macro-expansion. Plan complete, not yet started; it's the only open item in `docs/BACKLOG.md`.
