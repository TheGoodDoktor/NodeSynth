...
**Current status (2026-05-02):** Phases 0–5c complete (stereo + bundled presets + oversampling + Chorus + Flanger). Plus side-features: SID Player and MIDI Learn. SPSC command queue (UI → Audio) carries `SetParam` / `NoteOn` / `NoteOff`. Polyphonic subtractive synth target patch is the seeded default — `VirtualKbd` (events → queue) → `FVoiceAllocator` → `Osc` (per-voice) + `Adsr` (per-voice) → synthesised mixer → `Gain` (master trim 0.15) → `Output`. Holding 8 keys allocates 8 voices; a 9th note steals the oldest held with `Adsr` retrigger-from-current-level keeping it click-free. Node set: Oscillator (Sine/Saw/Square/Triangle/Noise with PolyBLEP), Gain, VCA, ADSR, SVF (TPT/ZDF form), Gate (manual toggle), MIDI Input, Virtual Keyboard, Output, Add, Multiply, Scale, Constant, Sample & Hold, LFO (Sine/Triangle/Saw/Square), Voice Allocator, Delay (2-second feedback line + Tone), Reverb (Freeverb), Waveshaper (TanhSoft / HardClip / SoftClip / Fold with dB Drive + Output), Clock (BPM gate), Sequencer (16-step external-clock-driven), Scope (oscilloscope tap), Meter (peak + RMS dBFS tap). Internal-only `_VoiceMixer` synthesised by the compiler when a per-voice Audio output feeds a mono Audio input. Front-end has node palette (drag-drop), node icons, ADSR envelope visualisation, persistent on-screen keyboard panel, File menu (New / Open / Save / Save As), Edit menu with Undo / Redo (`Ctrl+Z` / `Ctrl+Y` / `Ctrl+Shift+Z`, gated on `!WantTextInput`), per-voice flag toggle via right-click context menu with a `[poly]` header badge, custom property-panel UIs for ADSR (envelope plot), Sequencer (16-step grid), Scope (waveform), Meter (peak + RMS dBFS bars), Virtual Keyboard (piano + octave + mod). Patches are JSON files (`per_voice` field optional, defaults to false for back-compat); layout (`imgui.ini` + node-editor positions) persists to `~/.nodesynth/`. Catch2 test harness (160 tests, 63k+ assertions) covers graph, smoother, all DSP nodes, both SPSC rings under concurrent producer/consumer, command-queue dispatch + `SetParam` fan-out to per-voice clones, patch round-trip serialisation, `INode::Clone()` for every cloneable type, the per-voice flag, voice-allocator stealing policy (oldest fully-released → tailing → held), Compile partition with 8-voice polyphonic audio summation and independent per-voice ADSR release, every Phase 4 effect (delay timing + feedback decay + tone damping + fractional interp, reverb decay + room-size effect + bounded extremes, waveshaper shapes + dB Drive/Output + bounded fold, clock BPM, sequencer step advance + Reset + per-step state + clock-period measurement, scope ring wrap + snapshot, meter peak hold/decay + RMS), and undo/redo (round-trips for AddNode / RemoveNode / AddLink / SetParam / SetNodePerVoice, history capacity cap, redo-drop on new edit, RemoveNode-undo restores params + per-voice + incident links). Phase 5a + 5b additions: bundled presets browsable from File → Presets (eight initial entries across Init / Bass / Lead / Pad / FX, scanned from `<exe-dir>/presets/` and `~/.nodesynth/user_presets/`); audio buffers widened to 2-channel (`float[NumOutputs][NumChannels=2][BlockSize]`) with wire-level broadcast (mono producer's L buffer aliases the consumer's R input pointer, so existing mono nodes write only to channel 0 with zero migration churn); `INode::IsOutputStereo(port)` flag selects L→both broadcast vs L→L,R→R paired plumbing per link; `FReverb` and `FDelay` are true-stereo (Freeverb runs two parallel filter banks with the standard 23-sample stereo spread; Delay runs two independent feedback lines sharing Time / Feedback / Tone params). Audio callback reads both sink channels and routes to device L+R speakers. Phase 5c (oversampling + Chorus + Flanger) is next.
